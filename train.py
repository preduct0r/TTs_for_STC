...
def get_navec(df, path = '/mnt/c/Users/gaziz/navec_hudlit_v1_12B_500K_300d_100q.tar'):
    embs = []
    oov = []
...
def get_word2vec(df):
    ...
    model = word2vec.Word2Vec(words, size=100, window=2, min_count=0)
    for word in tqdm(df["word"].values, total=df.shape[0]):
        emb = model.wv[word]
        embs.append(emb)
    embs = np.array(embs)
    np.save('./word2vec.npy', embs)
    return embs
